## NYPD_Maps_new
## Change line 62 from column(7,
## Change line 108 column(5,
## Line 112/113
## Line 278 279 286 287
...
server<- function(input, output, session) {
    ...
    getColor <- function(values){
        lower <- min(values)
        upper <- max(values)
        mapPalette = leaflet::colorNumeric(rev(heat.colors(10)), c(lower, upper), 10)
        mapPalette(values)
        return(c(mapPalette, mapPalette(values)))
    }
            ...
            addLegend('bottomleft',
                      title = ifelse(input$colorby %in% c("arrests_weighted", "race_weighted"),
                                     ...
                                     ifelse( input$colorby %in% c("race_dist") ,
                                             "Population", "Arrests")),
                      pal = getColor(countryVar())[1], values = countryVar(), opacity = 0.7,
